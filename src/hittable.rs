...
impl HitList{
    ...
    pub fn hit(&self,r:&Ray,t_min:f64, t_max:f64, rec:&mut HitRecord)->bool{
        ...
        for var in (*self).clone().ptr.iter() {
            if var.unwrap().hit(r, t_min, closest_so_far, &mut temp_rec) {
                hit_anything = true;
    ...
    pub fn bounding_box(&self,t0:f64, t1:f64, output_box:&mut AABB) ->bool {
        ...
        let first_box = true;
        for  object in (*self).ptr {
            if  object.unwrap().bounding_box(t0, t1, &temp_box) {return false;}

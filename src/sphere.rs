...
impl Hittable for Sphere{
    ...
    let oc = r.position.clone() - self.center.clone();
    ...
    if  discriminant > 0.0{
        ...
        if temp < t_max && temp > t_min {
            rec.t = temp;
            rec.p = r.at(rec.t);
            let outward_normal:Vec3 = (rec.p.clone() - self.center.clone()) / self.radius.clone();
            rec.set_face_normal(r, &outward_normal);
            get_sphere_uv(&((rec.clone().p-self.center.clone())/self.radius.clone()), &mut rec.u, &mut rec.v);
